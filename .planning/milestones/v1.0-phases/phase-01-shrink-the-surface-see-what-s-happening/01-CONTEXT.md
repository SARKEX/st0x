...
### Plan-phase resolutions (2026-04-28, in response to RESEARCH.md open questions)

- **D-15:** OBS-03 dual-sink on the browser tier resolves to **Sentry + `console.error` JSON line** (NOT a server-relayed endpoint). `marketOrderExecution.ts` runs client-side; pino is server-only. The "pino half" of D-08 is reinterpreted: capture the failure with `Sentry.captureException(err, { extra: transcript })` AND emit a single `console.error('take-order failed', JSON.stringify(transcript))` line. PostHog session replay + Vercel's browser-console capture provide long-term searchability without an extra endpoint or extra network call per failure. Server-tier OBS-03 — if/when a server-relayed take-order path is ever introduced — would still go through pino as originally specified, but no such path exists today and Phase 1 does not introduce one. Acceptance test from D-08 (replay-from-one-log-entry) is satisfied by either the Sentry event JSON or the console-line JSON; the dev's tooling chooses.
- **D-16:** `src/lib/components/rewards/TokenSwapAnnouncementModal.svelte` is **kept** — it's a one-time token-migration announcement, not rewards UI. Move it out of the rewards directory (target: `src/lib/components/announcements/TokenSwapAnnouncementModal.svelte`) before deleting the rewards directory in DEPR-01. Retain `initTokenSwapAnnouncement` + `tokenSwapAnnouncementVisible` exports from `src/lib/stores/rewardsStore.ts` by extracting them into a new `src/lib/stores/announcementStore.ts` (planner picks exact filename per CONVENTIONS.md). All other rewards-store exports (`fetchRewardsData`, `userRewardsData`, `globalRewardsData`, `publicLeaderboardData`, etc.) delete with the rewards layer. The mount in `src/routes/(main)/+layout.svelte` survives but its import path updates to the new module location.

### Execution-phase amendments (2026-04-29, applied during /gsd-execute-phase 1)

- **D-17 (supersedes D-09 transport choice):** Switch chain-exhausted RPC alert transport from **Slack incoming webhook** to **Telegram bot**. Reason: the team uses Telegram for ops, not Slack. Code change: `src/lib/server/alerts.ts` POSTs to `https://api.telegram.org/bot<TOKEN>/sendMessage` with `{chat_id, text}` body. Two new env vars replace `OBSERVABILITY_ALERT_WEBHOOK_URL`: `OBSERVABILITY_ALERT_TELEGRAM_BOT_TOKEN` + `OBSERVABILITY_ALERT_TELEGRAM_CHAT_ID`. All other D-09 properties survive verbatim — fail-closed mild form (missing env → no-op + pino error log, does not kill cold-start), 3s `AbortSignal.timeout`, 512-char per-error `ERROR_TEXT_CAP`, payload includes function name + attempted RPC URLs + last error per RPC + request_id, no dedupe (every-occurrence). Function signature in `notifyChainExhausted` is unchanged so `rpcMetrics.ts` callers are unaffected. T-06-04 (Alchemy key in `rpc_url` field) trust-boundary argument is identical: Telegram channel = team trust boundary, same as the codebase. Phase 1 SUMMARY.md operational follow-up updated; 01-08 RUNBOOK template updated to document Telegram provisioning instead of Slack.
