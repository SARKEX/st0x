	...
	<div class="space-y-6 sm:space-y-8 p-4 sm:p-6">
		{#if myTokenBalance.length > 0}
			<div class="mb-6 sm:mb-8">
				<h2 class="mb-4 bg-gradient-to-r from-blue-500 to-purple-500 bg-clip-text text-xl sm:text-2xl font-bold text-transparent">
					My Vaults
				</h2>
				<div class="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
					{#each myTokenBalance as token}
						<div class="group relative overflow-hidden rounded-xl border border-white/5 bg-gray-700/30 p-4 sm:p-6 transition-all hover:border-blue-500/30 hover:bg-gray-700/40">
							...
							<div class="flex items-start gap-3 sm:gap-4">
								<div class="flex h-10 w-10 sm:h-12 sm:w-12 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-600/20 to-purple-700/20 text-lg sm:text-xl font-bold text-white ring-1 ring-white/10 backdrop-blur-sm">
								...
								<div class="flex-1">
									...
									<p class="text-xs sm:text-sm text-gray-400">{token.token.symbol ?? '???'}</p>
								</div>
							</div>
							...
							<div class="mt-4 space-y-2">
								<div class="flex items-center justify-between">
									<span class="text-xs sm:text-sm text-gray-400">Total Balance</span>
									<span class="text-base sm:text-lg font-semibold text-white">{token.balance}</span>
								</div>
								<div class="flex items-center justify-between">
									<span class="text-xs sm:text-sm text-gray-400">Price</span>
									<span class="text-xs sm:text-sm text-gray-300">${token.price}</span>
								</div>
								<div class="flex items-center justify-between">
									<span class="text-xs sm:text-sm text-gray-400">Estimated Value</span>
									<span class="text-xs sm:text-sm font-medium text-green-400">${token.estimatedValue}</span>
								</div>
								<div class="flex items-center justify-between">
									<span class="text-xs sm:text-sm text-gray-400">Vaults</span>
									<span class="text-xs sm:text-sm text-gray-300">{token.vaultIds.length}</span>
								</div>
							</div>
						</div>
					{/each}
		...
		<div class="mb-4 sm:mb-6">
			<label class="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:gap-2 w-full">
				<input
					type="checkbox"
					bind:checked={showMyVaults}
					class="h-4 w-4 rounded border-gray-600 bg-gray-700 text-blue-600 focus:ring-blue-500"
				/>
				<span class="text-xs sm:text-sm text-gray-300">Show only my vaults</span>
				<input
					type="checkbox"
					bind:checked={hideEmptyVaults}
					class="h-4 w-4 rounded border-gray-600 bg-gray-700 text-blue-600 focus:ring-blue-500"
				/>
				<span class="text-xs sm:text-sm text-gray-300">Hide empty vaults</span>
			</label>
		</div>
