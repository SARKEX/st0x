<script lang="ts">
  ...
  $: (function decode() {
    if (!receiptInformation) { data = null; return; }
    try {
...
{#if data}
  <Section>
    ...
    <pre class="max-h-96 overflow-auto rounded border border-white/10 bg-gray-900 p-3 text-xs text-gray-200">{JSON.stringify(data, null, 2)}</pre>
  </Section>
{:else}
