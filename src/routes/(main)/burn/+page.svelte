	...
	<div class="space-y-6 sm:space-y-8 p-4 sm:p-6">
		...
		<div class={SECTION_CLASSES}>
			<div class="mb-6 sm:mb-8">
				<h2 class="mb-4 bg-gradient-to-r from-orange-500 to-red-500 bg-clip-text text-xl sm:text-2xl font-bold text-transparent">
					Burn Partners
				</h2>
			</div>
				...
				<div class="{CARD_BASE_CLASSES} p-4 sm:p-6">
					<div class="space-y-4 sm:space-y-6">
						{#each BURN_ISSUERS as issuer}
							<div class="group relative overflow-hidden rounded-xl border border-white/5 bg-gray-700/30 p-4 sm:p-6 transition-all hover:border-orange-500/30 hover:bg-gray-700/40">
								<div class="absolute left-0 right-0 top-0 h-0.5 bg-gradient-to-r from-red-700 via-orange-600 to-yellow-500 opacity-0 transition-opacity group-hover:opacity-100" />
								<div class="flex flex-col sm:flex-row items-start gap-4 sm:gap-6">
									<div class="flex h-12 w-12 sm:h-14 sm:w-14 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-red-600/20 to-orange-700/20 text-lg sm:text-2xl font-bold text-white ring-1 ring-white/10 backdrop-blur-sm">
									...
									<div class="flex-1 space-y-2">
										<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
											...
											<a
												...
												class="text-xs sm:text-sm text-orange-500 transition-colors hover:text-orange-400"
											>
					...
					<div class="space-y-4 sm:space-y-6">
						...
						{#each $sfts as sft}
							{#each sft.withdraws.slice(0, 10) as withdraw}
								<div class="group relative mb-6 flex flex-col gap-4 rounded-lg border border-white/5 bg-gray-700/30 p-4 sm:p-6 transition-all hover:border-orange-500/30 hover:bg-gray-700/40">
									<div class="absolute left-0 right-0 top-0 h-0.5 bg-gradient-to-r from-red-700 via-orange-600 to-yellow-500 opacity-0 transition-opacity group-hover:opacity-100" />
									<!-- Status Badge -->
									<div class="flex flex-row gap-2 static sm:absolute sm:right-4 sm:top-4">
										{#if withdraw.id}
											<span class="rounded-full bg-gray-600 px-2 sm:px-3 py-1 text-xs font-semibold text-orange-300">Completed</span>
											<button
										...
										{:else}
											<span class="rounded-full bg-gray-600 px-2 sm:px-3 py-1 text-xs font-semibold text-orange-300">Processing</span>
										{/if}
									</div>
									<div class="flex items-center gap-3 sm:gap-4">
										<!-- Avatar -->
										<div class="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-xl bg-gradient-to-br from-red-600/20 to-orange-700/20 text-lg sm:text-2xl font-bold text-white ring-1 ring-white/10 backdrop-blur-sm">
											{sft.symbol?.slice(0, 2) ?? '??'}
										</div>
										<div>
											<div class="text-base sm:text-lg font-semibold text-white">{sft.name}</div>
											<div class="text-xs text-gray-400">Transfer ID: {truncateId(withdraw.id)}</div>
										</div>
									</div>
									...
									{#if withdraw.transaction.id}
										<div class="mt-4 rounded bg-red-900/50 px-4 py-2 text-xs text-orange-200 overflow-x-auto">
