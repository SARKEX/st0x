...
{#if showModal}
	...
	>
		<div class="w-full sm:w-[50rem] max-h-[80vh] overflow-y-auto rounded-xl border border-white/5 bg-gray-900/95 p-8 shadow-2xl backdrop-blur-md relative pointer-events-auto">
			...
			<h3 class="mb-6 text-2xl font-bold bg-gradient-to-r from-yellow-500 to-green-500 bg-clip-text text-transparent">Burn Information</h3>
            ...
            <div class="space-y-6">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div class="rounded-xl border border-white/5 bg-gray-700/30 p-4">
                        ...
                        <div class="text-white font-mono">{withdraw?.emitter.address.slice(0, 6)}...{withdraw?.emitter.address.slice(-4)}</div>
                    </div>
                    <div class="rounded-xl border border-white/5 bg-gray-700/30 p-4">
                        <div class="text-sm text-gray-400">Burn Timestamp</div>
                        <div class="text-white">
                            {withdraw?.timestamp ? new Date(Number(withdraw.timestamp) * 1000).toLocaleString() : 'Pending'}
                        </div>
                    </div>
                    <div class="rounded-xl border border-white/5 bg-gray-700/30 p-4">
                ...
                <div class="rounded-xl border border-white/5 bg-gray-700/30 p-4">
                    ...
                    <div class="text-white font-mono">{withdraw?.receipt.receiptId}</div>
                </div>
            </div>
		</div>
	</div>
