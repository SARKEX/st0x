<script lang="ts">
	...
	// Prevent background scroll when mobile sidebar is open. Client-only:
	// `document` is undefined during SSR, and docs are now server-rendered.
	$: if (browser) {
...
<!--
	Rendered unconditionally (no wallet gate). Docs need no wallet, so gating on
	`$wagmiConfig` — which is only set client-side — would have made SSR emit a
