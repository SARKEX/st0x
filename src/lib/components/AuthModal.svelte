<script lang="ts">
	...
	import { showAuthModal, closeAuthModal, loginWithPrivy, loginWithPrivyWallet, privyLoading } from '$lib/stores/privyStore';
	...
	function handlePrivyWalletLogin() {
		loginWithPrivyWallet();
		// Connect external wallet through Privy to get a smart account
	}
</script>
...
<Modal show={$showAuthModal} title="Connect or Log In" maxWidthClass="max-w-md" onClose={handleClose}>
	<div class="space-y-6">
		...
		<!-- Smart Wallet (EOA -> Smart Account) -->
		<div class="rounded-lg border border-gray-700 bg-gray-800/50 p-4">
			<div class="mb-3 flex items-center gap-3">
				<div class="rounded-full bg-gradient-to-br from-green-500/20 to-emerald-600/20 p-2">
				...
				<div>
					<h3 class="font-semibold text-white">Create Smart Account</h3>
					<p class="text-xs text-gray-400">Use MetaMask or Rabby to create a smart wallet</p>
				</div>
			</div>

			<Button
				on:click={handlePrivyWalletLogin}
				variant="secondary"
				fullWidth
				size="md"
				disabled={$privyLoading}
			>
				...
				{:else}
					Connect Wallet → Smart Account
				{/if}
			</Button>

			<p class="mt-2 text-center text-xs text-gray-500">
				Get a smart contract wallet powered by your existing EOA
			</p>
		</div>
		...
		<!-- Direct Wallet Connect (no Privy) -->
		<div class="rounded-lg border border-gray-700 bg-gray-800/50 p-4">
			<div class="mb-3 flex items-center gap-3">
				...
				<div>
					<h3 class="font-semibold text-white">Connect Wallet Directly</h3>
					<p class="text-xs text-gray-400">Use your EOA directly (advanced)</p>
				</div>
			</div>
