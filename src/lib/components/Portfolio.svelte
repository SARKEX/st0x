	...
	$: uniqueTokens = (() => {
		...
		for (const vault of vaults) {
			...
			if (!tokenMap.has(tokenKey)) {
				tokenMap.set(tokenKey, new Token({
					chainId: arbitrum.id,
...
<div class="space-y-6">
	<h2 class="bg-gradient-to-r from-blue-500 to-purple-500 bg-clip-text text-2xl font-bold text-transparent">
		My Portfolio
	</h2>
	...
	{#if $balancesQuery.isLoading}
		<div class="flex flex-col items-center justify-center rounded-xl border border-white/5 bg-gray-700/30 p-8 text-center">
			<div class="mb-4 flex h-16 w-16 items-center justify-center">
				<div class="h-8 w-8 animate-spin rounded-full border-4 border-gray-600 border-t-yellow-500"></div>
			</div>
	...
	{:else}
		<div class="flex flex-col items-center justify-center rounded-xl border border-white/5 bg-gray-700/30 p-8 text-center">
			<h3 class="mb-2 text-lg font-semibold text-white">No StoX balances found in your wallet</h3>
		</div>
	{/if}
</div>