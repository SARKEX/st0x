...
<div class="relative" bind:this={dropdownElement}>
	...
	<!-- Dropdown options -->
	{#if isOpen}
		<div class="absolute left-0 mt-1 w-full rounded-lg border border-white/10 bg-gray-800/90 shadow-xl z-50">
			<div class="max-h-60 overflow-y-auto py-1">
				{#each options as token (token.address)}
					<button
						type="button"
						class="flex w-full items-center gap-3 px-4 py-3 text-left text-white transition-colors rounded-md
							hover:bg-gray-700/70 focus:outline-none {selected?.address === token.address ? 'bg-gradient-to-r from-blue-600/20 to-purple-700/20' : ''}"
