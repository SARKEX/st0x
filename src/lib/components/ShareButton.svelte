<script lang="ts">
	...
	let shareText =
		'Trade tokenized equities, set up automated on-chain trading strategies, and more';
	let copied = false;
...
{#if showShareModal}
	...
	>
		<div
			class="mx-4 w-full max-w-sm rounded-xl border border-white/10 bg-gray-800/95 p-6 backdrop-blur-lg"
		>
			...
			<div class="mt-4 rounded-lg border border-white/10 bg-white/5 p-3">
				...
				<p class="truncate font-mono text-sm text-gray-300">{shareUrl}</p>
			</div>
		</div>
	</div>
{/if}
