		...
		<div class="space-y-4">
			...
			<div>
				<div class="mb-2 block text-sm font-medium text-gray-300">
					Limit Price
					<span class="ml-1 text-xs text-gray-400">({settlementLabel} per {assetToken.symbol})</span
					>
				</div>
				<Input
		...
		<details class="rounded-lg border border-white/[0.06] bg-gray-800/40 p-3">
			...
			<ol class="mt-2 space-y-1.5 text-xs text-gray-400">
				<li class="flex items-start gap-2">
					<span
						class="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-brand-gold-500/10 text-[10px] font-bold text-brand-gold-400"
						>1</span
					>
				...
				<li class="flex items-start gap-2">
					...
					<a href="/dashboard" class="text-brand-gold-500 hover:underline">Dashboard &gt; Vaults</a>
				</li>
			</ol>
		</details>
	...
	<div class="space-y-6">
		...
		<label class="flex items-center gap-3">
			<input
				...
				class="h-4 w-4 rounded border-gray-300 bg-gray-800 text-brand-gold-500 focus:ring-brand-gold-500"
			/>
			...
			<button
				...
				class={`flex-1 rounded-md px-4 py-2 text-sm font-medium transition focus:outline-none focus:ring-2 ${
					userAcknowledgesWarning
						? 'bg-brand-gold-500 text-gray-900 hover:bg-brand-gold-600 focus:ring-brand-gold-400'
