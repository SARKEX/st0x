<script lang="ts">
	...
	import { getDcaDeploymentArgs } from '$lib/services/orderDeployment';
	...
	const handleDsfDeploy = async () => {
		...
		if ($connected && selectedToken1 && selectedToken2 && usdcToken) {
			...
			const { deploymentArgs: cbBTCDeploymentArgs } = await getDcaDeploymentArgs($currentNetwork, {
			...
			const { deploymentArgs: tMSTRDeploymentArgs } = await getDcaDeploymentArgs($currentNetwork, {
				outputToken: usdcToken,
