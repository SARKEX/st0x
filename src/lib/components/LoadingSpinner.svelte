<script lang="ts">
	...
	const variantConfig = {
		inline: {
			...
			spinner:
				'relative animate-spin rounded-full border-4 border-transparent border-b-purple-700 border-l-green-500 border-r-blue-600 border-t-brand-gold-500',
		...
		button: {
			...
			spinner:
				'relative animate-spin rounded-full border-2 border-transparent border-b-purple-700 border-l-green-500 border-r-blue-600 border-t-brand-gold-500',
			text: 'text-sm'
		},
		dot: {
			container: 'inline-block',
			spinner: 'animate-pulse rounded-full bg-brand-gold-500',
			text: ''
		}
	};
...
{:else if variant === 'fullscreen'}
	<div class={currentVariant.container}>
		<div class="relative">
			<div
				class="absolute inset-0 animate-pulse rounded-full bg-gradient-to-r from-purple-700 via-blue-600 to-brand-gold-500 opacity-20"
			></div>
