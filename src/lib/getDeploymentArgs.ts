...
export const getDcaDeploymentArgs = async (args: DcaDeploymentArgs) => {
	const dcaStrategy = await (
		await fetch(
			'https://raw.githubusercontent.com/rainlanguage/rain.strategies/161fb82ff39ceb6b1ccacf8cbc3b97ef9977541f/src/auction-dca.rain'
		)
	).text();
...
export const getLimitOrderDeploymentArgs = async (args: LimitOrderDeploymentArgs) => {
	try {
		const dsfStrategy = await (
			await fetch(
				'https://raw.githubusercontent.com/rainlanguage/rain.strategies/161fb82ff39ceb6b1ccacf8cbc3b97ef9977541f/src/fixed-limit.rain'
			)
		).text();
...
export const getMarketMakingDeploymentArgs = async (args: MarketMakingDeploymentArgs) => {
	const dsfStrategy = await (
		await fetch(
			'https://raw.githubusercontent.com/rainlanguage/rain.strategies/161fb82ff39ceb6b1ccacf8cbc3b97ef9977541f/src/dynamic-spread.rain'
		)
	).text();
	const gui = await DotrainOrderGui.chooseDeployment(dsfStrategy, TARGET_NETWORK);
