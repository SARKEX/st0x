 ...
 */
async function getOrderBuilderFromRegistry(orderKey: string, raindexNetworkSlug: string) {
...
export const getLimitOrderDeploymentArgs = async (
	...
	const gui = await getOrderBuilderFromRegistry('fixed-limit', network.raindexNetworkSlug);
	...
	await gui.setDeposit('token2', formatUnits(args.depositAmount, args.outputToken.decimals));
