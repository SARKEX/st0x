...
function ScreenTabs({ screen, go }) {
  const tabs = [['home', 'Home'], ['trade', 'Trade'], ['earn', 'Earn'], ['dashboard', 'Dashboard'], ['metrics', 'Metrics']];
  return (
...
function ProductFrameWithRef({ screen, go, openDeposit, scrollRef }) {
  const label = { home: '', trade: 'trade/tNVDA', earn: 'earn', dashboard: 'dashboard', metrics: 'metrics' }[screen];
  return (
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#070b11] shadow-2xl shadow-black/60">
      ...
      <div ref={scrollRef} className="relative h-[768px] overflow-y-auto" data-screen-label={screen}>
        <Canvas screen={screen}>
          ...
          {screen === 'dashboard' && <Dashboard openDeposit={openDeposit} go={go} />}
          {screen === 'metrics' && <Metrics go={go} openDeposit={openDeposit} />}
        </Canvas>
      </div>
    </div>
  );
