  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      <svg className="chart-scroll absolute inset-y-0 left-0 h-full" style={{ width: '200%', animation: 'chartscroll 16s linear infinite' }} viewBox={`0 0 ${2 * W} ${H}`} preserveAspectRatio="none" aria-hidden="true">
        ...
        <path d={line} fill="none" stroke="#eab308" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" vectorEffect="non-scaling-stroke" />
      </svg>
      ...
      <div className="absolute bottom-4 left-5 flex items-center gap-1.5 text-[11px] text-gray-500">
        <span className="relative flex h-1.5 w-1.5"><span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-yellow-400 opacity-70"></span><span className="relative inline-flex h-1.5 w-1.5 rounded-full bg-yellow-400"></span></span>
    ...
    <div className="grid overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur md:grid-cols-2">
      ...
      <div className="flex flex-col p-5 sm:p-6">
        ...
        <div className="mt-3 flex items-center justify-center gap-2 text-xs text-gray-400">
          Trading on <span className="inline-block h-3 w-3 rounded-[3px] bg-[#2151f5]"></span> Base Mainnet
        </div>

        {/* primary: swap right here */}
        <button className="mt-3 w-full rounded-xl bg-gradient-to-r from-[#2f5bf0] to-[#9333ea] py-3 text-sm font-semibold text-white transition hover:brightness-110">
          Connect wallet
        </button>
        ...
        <button className="w-full rounded-xl bg-yellow-500 py-3 text-sm font-semibold text-black transition hover:bg-yellow-400">
