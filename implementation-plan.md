...
- Step through domains one by one (vaults → orderbook → trades → oracles) to move them to Query-based fetchers and new clients.
- After each migration, remove unused polling/controller code and update imports.
